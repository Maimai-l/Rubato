...
### 第 2 步:等自动停训(修正预报:很可能就是 50k 的解码腿;55k 几乎必停)

精确机制(train.py:1937,D98 核对):每次解码腿评测(每 5000 步)读**近 50 步滚动
平均 sem**;`滚动 sem ≤ 2.0 且 parseable < 0.80` **首次**满足即干净退出(写完最终
报告,非挂起)。45k 处滚动 sem 2.03-2.06,正贴门槛下行。单步 sem<2.0 不触发。
停下后贴回最后 20 行日志,直接进第 3 步。
(若到 60k 仍未自停 = sem 卡在 2.0 上方,也人工停一次进第 3 步。)
...
判据已预登记如下,先于数据,不许现场改。
【D88 补强】你的训练集自由解码诊断(70k:训练样本 parseable 也≈0)把"数据量/泛化"
解释排除,病灶=序列生成能力 —— 1c 正是对症杆,本门优先级上调。另请顺手贴一行词表
验证(任意 python):`import sentencepiece as spm; sp=spm.SentencePieceProcessor(model_file=r"D:\vscode_projects\ee_download\work\rubato_spm.model"); print([p for p in (sp.id_to_piece(i) for i in range(sp.get_piece_size())) if __import__("re").fullmatch(r"\d{1,2}/\d{1,2}", p)][:40])`
—— 时值 piece 若确为单 token,规划端再建 1d 时值加权(同 1b 机械)。

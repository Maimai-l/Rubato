...
## 终局判读与复训决定(2026-07-16,@64b9cd8,D29)

联合数据(24 条)的实际形态是**按源分层**,不是按对齐等级分层:
- **pdmx**:Δsem +0.03~+0.17(OK 桶均值 +0.12)—— 读音频,含音高。
- **nasap**:8 条全被审计判"错位",但 Δsem 均值 +0.07、**Δts 全体强正**(时间戳命中率
  翻倍以上)—— 模型在从中读到真实信号。结论:**能量相关审计在柔和钢琴上系统性误报**
  (踏板/连奏 → 能量起音弱),模型自身的 Δsem 才是更可靠的对齐探测器;
  "nASAP 对齐故障"降级为"偏噪/部分偏移,未致毒"。
- **maestro/AMT(唯一真异常)**:7 窗完美对齐(lag -10ms)但 Δsem 全 ≈0
  ——55% 的训练对,音高监督在纯文本模式空转;Δts 略正(时间在读一点)。
  注意本轮 8 窗同曲,需人群数据确认(探针池已修曲目去重)。

### 决定:**不重跑,从 ckpt 22000 复训**(参数不变)

依据:模型读音频的能力在主要方言(TAST/A2S)上被证实存活;没有发现单一致命 bug;
"平台期"的成分 = 任务硬 + AMT 分支音高监督空转(稀释)+ nasap 噪声,均非重启可解
(重启会在同样的数据上再学一遍同样的东西)。测量层已根治:每次 eval 自动跑
**多源 Δsem 探针**(固定三样本池,逐 eval 可比)。
...
### 下次复盘:step 30000(预登记仪表)

1. 多源 Δsem 三条曲线:pdmx/nasap 应持平或上升;**maestro 若仍 ≈0 → 开 AMT 专项卡**
   (窗账复核 + 考虑 AMT 音高监督的结构性修正)。
2. A2S 训练损失 22000→30000 斜率(基线 -0.017/500,若 Δsem 上升它应跟着变陡)。
3. parseable/样例继续观察但不作主指标(生成侧成熟本来就晚于教师强制指标)。

后台任务(不阻塞训练):nASAP 窗口时间轴复核降级为可选;audit 工具的能量包络
对柔和钢琴的失敏记入工具 docstring,后续版本可换谱通量/色度特征。

...
| ss= rr= | scheduled sampling 两遍法(D102,--sched-sampling>0 才出现):第二遍 sem / 实际替换率 | **开启后 loss 列换义 = 第二遍损失**(sem/ts/pv 列仍为第一遍口径);ss 随训练降 = 模型学会接住自己的错误;rr 应恒 ≈ 目标率 |
...
**拒因行**(D43 起):
    ...
    eval 拒因(样本数): 兜底=41 DYCK=3 MEASURE=2 通过=4 /共48

每类校验拦下多少个样本(一样本可入多类;「兜底」= 模型输出被拒后落到兜底常量的样本)。
这是 parseable 卡点的直方图 —— 哪类最大,下一步就修哪类。

**多源探针行 D43 新增字段**:`Δpitch=`/`真pitch=` —— 音高 token(N60/n60/C4 这类原子
字形)单独的教师强制命中率之差(真音频 − 静音)。比 Δsem 更锋利:Δsem 被 velocity/结构
token 稀释,**AMT 有没有从音频里读音高,看 Δpitch**。
...
- raw_ned= / raw_scored=(D89 优雅针,LEGATO 测量制度移植):**全样本**进分母的自由生成
  内容 NED —— 验证器拒绝的样本用逐窗原文剥戳拼接打分,空输出=1.0。parseable 悬崖之前
  看自由生成进步的唯一连续读数,应随训练下降;不参与选优,不冒充 OMR-NED。

...
## 6b. Step 0b 复盘:语料到不了 30 万行 —— 我的目标数错了,判据要换

**执行端实测**: 16,771 曲(去重后)、2.3 段/曲、1.8 有效标签/曲 → 语料上限 **~56k 行**,
远不到我写的 30 万。执行端诊断正确,但在调错杠杆(min_measures=2/重叠只多 10-20%)。

**根因(缩水规模的固有张力,我上次没讲清)**:
- 论文 tokenizer 语料 = PDMX **~250k 谱 → 100 万 utterance** → 3571 可学习 piece。
- SPEC D4 缩水到 12-20k 曲(~6-9 万段)。**可学习 piece 上限 = 语料 distinct 子串数**,
  缩水池只能支撑远少于 3571 → **vocab==8000 在这个规模不可达**。这不是 bug。
- 我给的"30 万行 / vocab 8000 / learnable 3571"是照搬论文规模,与 SPEC 的缩水池自相矛盾。

**判据换成真正重要的那个**:
- ❌ 不再要求 vocab==8000 / learnable==3571(缩水规模到不了,是 D4 已知偏离)。
- ✅ **check_glyph_coverage 的 split_rate<0.30**(常见字形 C4/1/4/|4/4k0 是否原子)。
  词表小(~5k)但字形原子 = 模型可用,只是序列略长。上一轮"字形 100% 分裂"多半是
  tokenizer 没正确注入 user_defined 的假象,用修好的 train_unigram 重测。
- `train_unigram` 已改:目标词表过高会自动回退到语料上限重训,返回实际词表 + warning,不卡死。

**扩语料的安全杠杆(想逼近 8000,按性价比排序)**:
1. 并入 **nASAP A2S/A2S_lite 文本**(论文语料本就含它,你有 ~18.8M chars,白拿)。
2. **tokenizer 语料用未按 work_key 去重的训练侧全部曲**(去重是给 train/val/test 隔离的,
   不是给 tokenizer 语料的;用 18,382 甚至放宽 250k→18k 的预过滤能再多些)。
3. **重叠切段**(segment_score_overlap)多 ~1.5-2x utterance。

**⚠ 需要用户决策的分叉(执行端别自己定)**:
上述杠杆用满大概到 ~100-150k 行、词表 ~5-6k。若用户要更接近论文 8000,只能扩渲染池
(向 250k 靠),代价是大量渲染时间。**推荐**:接受缩水词表(~5-6k)+ split_rate<0.30,
先把管线跑通;词表大小是 D4 缩水的已知代价,不阻塞训练。把这个分叉抛给用户定。

## 7. 一句话给执行端

**头号任务是修地基:S3 过滤 + S4 渲染 + PDMX A2S 标签做到 ≥30 万行语料 → 训真 8000 tokenizer
(learnable==3571)→ 重建模型。这之前建的一切(4760 tokenizer + 其上的模型/训练)全作废。
并行地:用现成真实数据(MAESTRO AMT 1,276)跑 Step E 真过拟合(loss<0.05)证明 #1 机制、
用 diagnose_match 把 #6 匹配率验到 >80%。一切真实数据适配 commit 进仓库。数据备齐前不开全量训练。**

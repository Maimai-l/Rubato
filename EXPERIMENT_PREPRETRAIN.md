...
- 强化一档(D93,取代原"暂停窗复评"计划):语料 200k→400k(v2 新种子)、步数
  20k→40k、门内置 n=48 自由续写(--free-eval-n 48);**门(先于数据):末次评测
  parseable ≥29/48 且 DYCK≈0**。过门 round-3 用 v2 工件;不过门用已合格 v1,
  只强化这一次,不追"完美"(预训练装偏置,不练满分;正训会继续塑造)。
- **v2 结果与判定(2026-08-05,D93/D94)**:40k 步跑完(avg50 1.888);末次 n=48
  评测 36/48 可解析 ✓ 但 **DYCK=5 ≠0 → 按门拒收**,round-3 用 v1。轨迹显示 30k 处
  曾达 45/48・DYCK2(最优态)但被"只存末态"覆盖丢失 —— 教训已修:预训练器新增
  **最优旁存**(*_best.pt,按 (可解析数,−DYCK) 选优,跨恢复保序,meta 自带健康门),
  将来任何重跑自动保留最优检查点。门设计自评(D94):候选间必须同尺比较
  (v1 只有 n=4 读数,v2 有 n=48,"拒 v2 用 v1"隐含未经检验的 v1≥v2)。
- **门修订(2026-08-05,D95;诚实条款:受已见 v2 数据启发,非纯先验)**:
  "DYCK=0"废除,降级为选优第二键;底线仅可解析 ≥29/48。选拔制取代阈值制:
  {v1@n48, v2-final, v3-best, v3-final} 同尺取 free_eval_rank 最大者进 round-3
  (v3 = v2 配置带旁存重跑,找回被丢的最优态;选优对象的数据在修订时均未出生,
  预登记先于它们)。round-3 的 20k 三判据与冻结基线不受影响。

...
本驱动器(替代 SOP P5c 的直跑):
  ...
  2. 待跑索引切成连续块(缺省 8 首/块),每块起一个 s7 子进程(--offset/--limit)写【临时
     文件】,成功才并入主标签文件 —— append 语义由驱动器独占,子进程互不踩;
  3. 块超时/失败 → 自动二分为单曲重试:好曲全保,只有真坏曲记名跳过(不再整块陪葬);
  4. 超时子进程由 subprocess.run 杀死,不留僵尸;
  5. 主标签文件已存在但无状态文件(来历不明,如手工分块的半截产物)→ 改名 .orphan 留档,
     从头重跑 —— 不删任何东西,也不停下来等人。
  6. 收尾打印 nasap_segments=/nasap_failed=(SOP 解析)+ 失败清单落 reports/s7_resilient.json。

用法(执行端;sop_next --go 的 P5c 就是它,一般不用手动跑):
  python scripts/s7_resilient.py                  # 跑/续跑

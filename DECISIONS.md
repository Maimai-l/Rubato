...
| # | 问题 | 背景 |
|---|------|------|
| O1 | tokenizer 语料是否只用 train split?(现状:PDMX 文本标签行不带 split,val/test 的 A2S 文本也进语料) | 论文 §3.2 只说"fit 在 A2S+A2S_lite 语料";文本级词表污染算不算泄漏需要定。若要 train-only:标签行需从 manifest 关联 split 字段,rebuild_corpus 加过滤 |
| O2 | nASAP 分段是否也改"时间唯一上限"?(现状:保持论文明确的 4–32 重叠窗) | 对标评测,建议不改;用户可否决 |

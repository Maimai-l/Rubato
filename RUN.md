...
> ⚠⚠【2026-07-14 起已进入训练期】:数据管线 P0-P8 已全绿,本文件是数据期的历史参考。
> 当前任务看 **EXPERIMENT_H1.md**(入口一律看 EXECUTOR.md 的「当前阶段」)。
> ⚠【2026-07-11 起,当前任务 = 数据重跑】:只跑 `python scripts/sop_next.py --go`,
> 它会从 P0 自动干到 P7,【不等任何人发话】;只有真失败才停(贴回后修复再 --go 续)。
> 详见 **SOP_RERUN.md** 头部。本文件的分步命令是背景与判据参考。
...
5. **连续推进,不等确认:** `git pull` 后跑 `sop_next.py --go`,它自动一路干到底。贴回块随跑随发,
   【不要停下来等任何人回复】。唯一停下的理由 = 真失败(贴 traceback 等修复)。

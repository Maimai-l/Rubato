...
## 判决:数据无罪,AMT 不读音频是模型侧现象(2026-07-17,@94c9881,D31)

音高审计 16 样本:**14 PITCH_OK / 1 MISMATCH / 1 AMBIG → 音高OK**。
唯一 MISMATCH 是时间偏移 -1080ms 的样本(时间错则音高-时间对应断,自洽);
起音对齐 13/16 OK。**maestro 的音频↔标签在时间和音高两个维度都干净。**

按本卡预登记判读:AMT 不读音频 = 模型侧现象 —— AMT 文本自预测性太强(静音基线
sem=0.72),音频的边际梯度小,是模型**最后**才会学的东西,不是 bug。处置:
**不动数据、不动训练,专项关案**;38000 复盘看 AMT 的 Δsem 是否脱零,仍零再议
结构性修正(候选已备:AMT 样本平滑/混比微调,届时另开卡)。

至此三源数据全部定性:pdmx 干净(15/16)、maestro 干净(时间+音高双查)、
nasap 偏噪但有真实信号(能量审计对柔和钢琴偏严,模型 Δsem/Δts 为准)。
**数据侧调查全线关闭。**
